...
.regc{font-weight:600;color:var(--mut);font-size:12px}
.reglist{margin-top:14px;border-top:1px dashed var(--line);padding-top:12px}
.reglist summary{cursor:pointer;font-size:12.5px;font-weight:700;color:var(--blue);list-style:none;outline:none}
.reglist summary::-webkit-details-marker{display:none}
.reglist summary:before{content:'\25B8  ';color:var(--red)}
.reglist[open] summary:before{content:'\25BE  '}
.regchips{display:flex;flex-wrap:wrap;gap:7px;margin-top:12px}
.regchips span{font-size:11px;padding:5px 10px;border-radius:16px;background:var(--soft);border:1px solid var(--line);color:var(--mut)}
.mlist{margin-top:14px;display:grid;gap:16px}
.mrow{border-left:2px solid var(--line);padding-left:14px}
.mrow .mname{font-family:'Sora';font-weight:700;font-size:13.5px;color:var(--ink);margin-bottom:7px;display:flex;align-items:center;flex-wrap:wrap;gap:4px}
.mrow .mname em{color:var(--mut);font-style:normal;font-weight:600;font-size:12px}
...
function buildDisp(d){
  ...
  d.recs.forEach(r=>{
    ...
    else if(low.startsWith('ministerio de')||low.startsWith('ministerio del'))key='min';
    if(key){(fam[key]=fam[key]||[]).push(r);}else out.push(r);
  });
  Object.keys(fam).forEach(k=>{
    const arr=fam[k];
    if(arr.length>=3){
      out.push({_group:k,institucion:k==='reg'?'Gobiernos Regionales':'Ministerios',
        estado_rec:arr[0].estado_rec,acciones:arr[0].acciones,_count:arr.length,
        _totalRecs:arr.reduce((s,x)=>s+(Number(x.total)||0),0),
        total:k==='reg'?arr[0].total:undefined,texto:arr[0].texto,
        _items:arr.map(x=>({name:(x.institucion||'').trim(),total:x.total,texto:x.texto,estado_rec:x.estado_rec}))});
    }else{arr.forEach(r=>out.push(r));}
  });
  return out;
}
...
function detailHTML(i){
  ...
  const mx=Math.max(...disp.filter(r=>r._group!=='min').map(r=>Number(r.total)||0),1);
  const rx=/^[IVX0-9]+[\.\)]\s*/;
  const recsHtml=disp.map(r=>{
    ...
    if(r._group==='reg'){
      const items=splitRecs(r.texto).map(x=>`<li>${x.replace(rx,'')}</li>`).join('');
      ...
      return `<div class="rec ${cls} reveal"><div class="head"><div class="ring" style="--q:${q}"><b>${r.total||'–'}</b></div>
        ...
        <div class="pill ${cls}">${r.estado_rec||'—'}</div></div><ul>${items}</ul>
        <details class="reglist"><summary>Ver los ${r._count} gobiernos regionales</summary><div class="regchips">${names}</div></details>${act}</div>`;
    }
    if(r._group==='min'){
      const list=r._items.map(x=>`<div class="mrow"><div class="mname">${x.name} <em>(${x.total})</em> <span class="pill ${sc(x.estado_rec)}" style="margin-left:6px">${x.estado_rec||'—'}</span></div><ul>${splitRecs(x.texto).map(b=>`<li>${b.replace(rx,'')}</li>`).join('')}</ul></div>`).join('');
      return `<div class="rec ${cls} reveal"><div class="head"><div class="ring" style="--q:100"><b>${r._count}</b></div>
        <div class="inst"><div class="who">Ministerios <span class="regc">(${r._count})</span></div><div class="meta">${r._totalRecs} recomendaciones · notificados a ${r._count} ministerios</div></div>
        <div class="pill ${cls}">${r.estado_rec||'—'}</div></div>
        <details class="reglist"><summary>Ver los ${r._count} ministerios notificados</summary><div class="mlist">${list}</div></details>${act}</div>`;
    }
    const items=splitRecs(r.texto).map(x=>`<li>${x.replace(rx,'')}</li>`).join('');
    const q=Math.round((Number(r.total)||0)/mx*100);
    return `<div class="rec ${cls} reveal"><div class="head"><div class="ring" style="--q:${q}"><b>${r.total||'–'}</b></div>
