...
# labels exactos solicitados por el usuario
labels = {
 1:"Informe Defensorial N°001-2023-DP/ACSGO",
 2:"Informe Defensorial N°273",
 3:"Informe Defensorial 278",
 4:"Informe Defensorial 284",
 5:"Informe Defensorial 279",
}
for d in data:
    d['label'] = labels[d['n']]
...
<link href="https://fonts.googleapis.com/css2?family=Sora:wght@400;600;700;800&family=Inter:wght@400;500;600;700&family=Space+Grotesk:wght@500;700&display=swap" rel="stylesheet">
<style>
:root{
...
.grid{position:fixed;inset:0;z-index:0;opacity:.5;background-image:linear-gradient(rgba(10,86,168,.05) 1px,transparent 1px),linear-gradient(90deg,rgba(10,86,168,.05) 1px,transparent 1px);background-size:46px 46px;mask:radial-gradient(circle at 50% 30%,#000,transparent 80%)}
...
.top{grid-column:1/3;display:flex;align-items:center;gap:20px;padding:14px 34px;border-bottom:1px solid var(--line);background:rgba(255,255,255,.85);backdrop-filter:blur(10px)}
...
.top .spacer{flex:1}
.anniv{height:66px;width:auto;border-radius:10px}
...
.nav-card{position:relative;display:block;width:100%;text-align:left;cursor:pointer;border:1px solid var(--line);background:#fff;border-radius:16px;padding:15px 16px 15px 18px;margin-bottom:12px;color:var(--ink);box-shadow:0 4px 18px rgba(16,42,80,.05);transition:.34s cubic-bezier(.2,.7,.2,1);overflow:hidden}
.nav-card:hover{transform:translateY(-3px);box-shadow:var(--sh)}
.nav-card .num{font-family:'Sora';font-weight:700;font-size:13.5px;margin:0 0 6px;line-height:1.3;color:var(--blue-d)}
...
.hero .bignum{position:absolute;right:26px;top:-26px;font-family:'Space Grotesk';font-weight:700;font-size:190px;line-height:1;color:transparent;-webkit-text-stroke:2px #e6eefa;pointer-events:none;user-select:none}
...
.doc-num{font-family:'Space Grotesk';font-size:15px;color:var(--blue-d);font-weight:700;margin:16px 0 8px;letter-spacing:.3px}
...
<div class="app">
  ...
  <aside class="rail"><h2>Navegador de informes</h2><div id="nav"></div></aside>
...
DOCS.forEach((d,i)=>{
  ...
  b.innerHTML=`<div class="num">${d.label}</div><div class="sub">${d.titulo.replace(/\n/g,' ')}</div>
...
function show(i){
  ...
  stage.innerHTML=`<div class="fade">
    ...
    <div class="footnav"><button class="btn" id="prev">&larr; Anterior</button><div class="dots" id="dots"></div><button class="btn" id="next">Siguiente &rarr;</button></div></div>`;
