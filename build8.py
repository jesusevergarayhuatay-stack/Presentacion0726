...
function detailHTML(i){
  const d=DOCS[i];
  const disp=[];const reg=[];
  d.recs.forEach(r=>{((r.institucion||'').trim().toLowerCase().startsWith('gobierno regional')?reg:disp).push(r);});
  if(reg.length){disp.push({institucion:'Gobiernos Regionales',total:reg[0].total,texto:reg[0].texto,estado_rec:reg[0].estado_rec,acciones:reg[0].acciones,_count:reg.length,_names:reg.map(x=>(x.institucion||'').trim())});}
  const mx=Math.max(...disp.map(r=>Number(r.total)||0),1);
  const recsHtml=disp.map(r=>{
    ...
    return `<div class="rec ${cls} reveal"><div class="head"><div class="ring" style="--q:${q}"><b>${r.total||'–'}</b></div>
      <div class="inst"><div class="who">${r.institucion||'—'}${r._count?' <span class=\'regc\'>('+r._count+')</span>':''}</div><div class="meta">${r._count?(r.total||0)+' recomendaciones aplicadas a los '+r._count+' gobiernos regionales':(r.total||0)+' recomendación(es) dirigidas'}</div></div>
      <div class="pill ${cls}">${r.estado_rec||'—'}</div></div><ul>${items}</ul>${r._count?'<details class=\'reglist\'><summary>Ver los '+r._count+' gobiernos regionales</summary><div class=\'regchips\'>'+r._names.map(n=>'<span>'+n+'</span>').join('')+'</div></details>':''}${act}</div>`;
  }).join('');
  const ds=docState(d);
